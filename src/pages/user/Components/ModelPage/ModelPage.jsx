...
import modelImg from "./model.svg"
...
const ModelCard = ({ project }) => (
  <div className="bg-white border border-blue-200 rounded-xl shadow-sm hover:shadow-md transition-all p-5 space-y-4">
    ...
    <div>
      ...
      <h3 className="text-lg font-semibold text-blue-700 truncate">{project.model}</h3>
    </div>
  ...
  return (
    <motion.header className="bg-gray-100 p-4 relative">
      <div className="container mx-auto flex flex-col lg:flex-row items-center justify-between">
        ...
        <motion.img src={modelImg} alt="model" className="w-64 h-auto" />
...
const ModelPage = () => {
  ...
  const filters = [
    "Task", "Data Type", "Model Type", "Creator/Publisher",
    ...
    if (activeFilters.length > 0) {
      filtered = filtered.filter((proj) =>
        activeFilters.every((f) =>
          proj.model?.toLowerCase().includes(f.toLowerCase()) ||
  ...
  return (
    <div className="max-w-7xl mx-auto flex gap-3 mt-8">
      ...
      <div className="flex-1">
        <Header
          title="Models"
          subtitle="Explore all shared models across users"
